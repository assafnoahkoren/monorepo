...
export class AppController {
  ...
  runScenario(
    @Body() body: RunScenarioBody,
    @Query() query: RunScenarioQuery,
  ): any {
    const residences = body.residences.map((residence) => {
      return {
        id:
          residence.id?.split('-')[0].trim() ||
          residence.name?.split('-')[0].trim(),
  ...
  checkAvailability(@Query() query: CheckAvailabilityQuery): any {
    ...
    allocation = allocation.filter((a) => a.roomsLeft > 0);
  ...
  reserve(@Query() query: ReserveQuery): any {
    ...
    if (allocation[query.residence] === undefined) {
      if (Object.keys(allocation).length === 0) {
        ...
        const residenceKeyToReserve = allocation[0].name;
        const roomsNeeded = Math.ceil(query.amount / state.persons_in_rooms);
        const residenceIndex = state.allocationsLeft[
          query.settlement
        ].findIndex((a) => a.name === residenceKeyToReserve);
        state.allocationsLeft[query.settlement][residenceIndex].roomsLeft -=
          roomsNeeded;
    ...
    const roomsNeeded = Math.ceil(query.amount / state.persons_in_rooms);
    const residenceIndex = state.allocationsLeft[query.settlement].findIndex(
      (a) => a.name === query.residence,
    );
    state.allocationsLeft[query.settlement][residenceIndex].roomsLeft -=
      roomsNeeded;
    const id = (GLOBAL_SEQUENCE++).toString();
